...
import cv2
import numpy as np
from flask import Flask, render_template, Response, request, redirect, url_for, session, jsonify, flash, get_flashed_messages
...
def _log_track_data(track_id, ts, centroid_x, centroid_y, speed, direction, anomaly_reason):
    try:
        ...
        print(f"Error logging track data: {e}")
    finally:
...
if camera_index is None: raise RuntimeError("No working camera found.")
...
def _match_and_assign_ids(detected_centroids):
    ...
    unmatched = []
    used_tids = set()
    if not detected_centroids: return matches, []
...
def _compute_speed_and_direction(history):
    if len(history) < 2: return 0.0, 0.0, 0.0, "STILL", []
    t0, p0 = history[0]
    t1, p1 = history[-1]
    dt = t1 - t0
    if dt <= 0.0001: return 0.0, 0.0, 0.0, "STILL", []
    ...
    if abs(vx) < 2 and abs(vy) < 2:
        ...
        if abs(vx) >= abs(vy): d = "RIGHT" if vx > 0 else "LEFT"
        else: d = "DOWN" if vy > 0 else "UP"
...
def _check_for_anomalies(track_id, speed, direction, history, people_report_count):
    ...
    if people_report_count >= ANOMALY_CROWD_COUNT and speed > ANOMALY_STAMPEDE_SPEED:
        if anomaly_reason: anomaly_reason += "/EXTREME_SPEED"
        else: anomaly_reason = "EXTREME_SPEED"
    if people_report_count >= ANOMALY_DENSITY_THRESHOLD:
        if anomaly_reason: anomaly_reason += "/HIGH_DENSITY"
        else: anomaly_reason = "HIGH_DENSITY"
    return anomaly_reason is not None, anomaly_reason

# ---------------- Inference & annotate ----------------
def safe_infer_and_annotate(frame):
    global _tracks, latest_people_report, latest_crowd_conflict, latest_density_info
    try:
        ...
        try:
            ...
            with frame_lock: latest_crowd_conflict = False
    ...
    except Exception: xyxy = []
    
    ids_arr = None
    try: ids_arr = boxes.id.cpu().numpy()
    except Exception: ids_arr = None
    ...
    for i, box in enumerate(xyxy):
        try: x1, y1, x2, y2 = [int(v) for v in box]
        ...
        if ids_arr is not None and i < len(ids_arr):
            try:
                val = ids_arr[i]
                if not np.isnan(val): tid = int(val)
    ...
    for (cx, cy), tid_in, bbox in raw_items:
        if tid_in is not None: _update_track(tid_in, (cx, cy), now)
        else: centroids_no_id.append((cx, cy))
    
    if centroids_no_id:
        matches, _ = _match_and_assign_ids(centroids_no_id)
        for tid, centroid in matches: _update_track(tid, centroid, now)

    with _tracks_lock: tracks_snapshot = dict(_tracks)
    ...
    for (cx, cy), tid_in, bbox in raw_items:
        ...
        _log_track_data(chosen_tid, now, cx, cy, round(float(speed_px_s), 2), direction, anomaly_reason if is_anomaly else None)

        if direction in directions_map: directions_map[direction] += 1
        vx_sum += vx
        vy_sum += vy
        person_count += 1
        ...
        if anomaly_reason: label += f" | {anomaly_reason}"
    ...
    # 🔥 Calculate Density
    density_val = min(round((person_count / MAX_CAPACITY) * 100), 100)
    if density_val < 40:
        d_level = "Low"; d_color = "#28a745"
    elif density_val < 75:
        d_level = "Medium"; d_color = "#fd7e14"
    elif density_val < 90:
        d_level = "High"; d_color = "#dc3545"
    else:
        d_level = "Critical"; d_color = "#8b0000"

    _age_and_cleanup(TRACKER_MAX_AGE)
    person_count=len(people_report)
    
    cv2.putText(annotated, f"Persons: {person_count}", (10,40), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0,255,0), 2)
    cv2.putText(annotated, f"Density: {density_val}% ({d_level})", (10,80), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,0), 2)
    if crowd_conflict:
        cv2.putText(annotated, "ALERT: CROWD CONFLICT", (10,120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2)

    with frame_lock:
        latest_people_report = people_report.copy()
        latest_crowd_conflict = crowd_conflict
        latest_density_info = {"value": density_val, "level": d_level, "color": d_color}
...
def init_history_db_connection():
    conn = sqlite3.connect(HISTORY_DB_PATH)
    conn.row_factory = sqlite3.Row
    return conn
...
def video_feed():
    def generator():
        while True:
            with frame_lock: frame = latest_frame_jpg
            if frame is None:
                time.sleep(0.05)
                continue
...
def history_page():
    ...
    cur = conn.execute("SELECT * FROM tracks_history WHERE timestamp > ? ORDER BY timestamp DESC", (one_day_ago,))
...
def history_data_api():
    ...
    conn = init_history_db_connection()
    cur = conn.execute("SELECT * FROM tracks_history ORDER BY timestamp DESC LIMIT ?", (limit,))
